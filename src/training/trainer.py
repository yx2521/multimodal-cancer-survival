...
def train(datasets, args):
    ...
    # WANDB writer
    import wandb
    # writer = wandb.init(project="mmp_final", config=vars(args)) if hasattr(args, 'wandb_project') and args.wandb_project else None
    fold_k = getattr(args, 'split_k', 0)
    ...
    for epoch in range(args.max_epochs):
        ...
        if writer:
            writer.log({
                ...
                'train_c_index': train_results.get('c_index', 0),
                'learning_rate': train_results.get('lr', 0)
            })
        #########
        ...
        if 'val' in datasets.keys():
            ...
            if writer:
                writer.log({
                    ...
                    'val_c_index': val_results.get('c_index', 0)
                })
    ...
    for k, loader in datasets.items():
        ...
        if writer:
            writer.log({
                ...
                f'final_{k}_loss': results[k].get('loss', 0)
            })
        ######
