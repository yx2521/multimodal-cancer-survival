...
def cluster(data_loader, n_proto, n_iter, n_init=5, feature_dim=1024, n_proto_patches=50000, mode='kmeans', use_cuda=False):
    ...
    #     data = batch['img']  # 或 batch['features']
        
    #     # 调试每个batch
    #     if batch_idx < 3:  # 前3个batch
    #         print(f"Batch {batch_idx} 调试:")
    #         print(f"  形状: {data.shape}")
    #         print(f"  统计: mean={data.mean():.6f}, std={data.std():.6f}")
    ...
    if mode == 'kmeans':
        ...
        # # K-means之前最终检查
        # print("K-means输入数据最终检查:")
        # print(f"  形状: {patches.shape}")
        # print(f"  统计: mean={patches.mean():.6f}, std={patches.std():.6f}")
        
        # if patches.std() < 1e-6:
        #     print("🚨 K-means输入数据方差太小！问题在这里！")
