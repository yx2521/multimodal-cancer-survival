...
def extract_patching_info(s):    
    # clam_gen_XXXX
    match = re.search(r"clam_gen_(\d+)", s)
    if match:
        patch_size = int(match.group(1))  # 1024/2048
        mag = 20  # default
        return mag, patch_size
...
def _read_gene(omics_dir, split):
    ...
    if os.path.isfile(split_path):
        ...
        df = df.reset_index()
        df = df.rename(columns={'sample': 'case_id'})
    else:
        raise FileNotFoundError(f"{split_path} not found!")
    
    df['case_id'] = df['case_id'].apply(lambda x: '-'.join(x.split('-')[:-1]) if len(x.split('-')) > 3 else x)
