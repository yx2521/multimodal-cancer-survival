...
for dataroot_path in "${dataroots[@]}"; do
  feat_dir=${dataroot_path}/TCGA-LUNG/clam_gen_2048/uni_v1
  if ! test -d $feat_dir
  then
    continue
  fi
...
if [[ $load_proto -eq 1 ]]; then
  cmd="$cmd --load_proto \\
  --proto_path "splits/${split_dir}/prototypes/prototypes_c${out_size}_clam_gen_2048_kmeans_num_${proto_num_samples}.pkl" \\
  "
fi
