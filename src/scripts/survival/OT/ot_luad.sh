...
# Multimodal args
model_mm_type='coattn_mot'    # 'coattn', 'coattn_mot', 'survpath', 'histo', 'gene'
append_embed='random'
histo_agg='mean'
num_coattn_layers=1
...
for dataroot_path in "${dataroots[@]}"; do
  feat_dir=${dataroot_path}/TCGA-LUNG/clam_gen_1024/uni_v1
  if ! test -d $feat_dir
  then
    continue
  fi
...
if [[ $load_proto -eq 1 ]]; then
  cmd="$cmd --load_proto \\
  --proto_path "splits/${split_dir}/prototypes/prototypes_c${out_size}_clam_gen_1024_kmeans_num_${proto_num_samples}.pkl" \\
  "
fi
